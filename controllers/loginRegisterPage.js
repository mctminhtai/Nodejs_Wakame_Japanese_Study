...
exports.post_change_pw = function (req, res, next) {
    ...
    bcrypt.compare(req.body.currentpw, req.user.dataValues.password).then((result) => {
        if (result && (req.body.newpw == req.body.re_newpw)) {
            ...
            res.render('resetpw', { message: 'Sai mật khẩu hiện tại hoặc mật khẩu xác nhận và mật khẩu mới không giống nhau' });
        }
    })
}
...
exports.get_test = async function (req, res, next) {
    ...
    await models.TAG.create({ TEN_TAG: 'N1' });
    await models.TAG.create({ TEN_TAG: 'N2' });
    await models.TAG.create({ TEN_TAG: 'N3' });
    await models.TAG.create({ TEN_TAG: 'N4' });
    await models.TAG.create({ TEN_TAG: 'N5' });
    await models.CATEGORY.create({ name: 'Học Tập' });
    await models.CATEGORY.create({ name: 'Văn Hoá' });
    await models.CATEGORY.create({ name: 'Sự Kiện CLB Wakame' });
    await models.CATEGORY.create({ name: 'Tài Liệu' });
    await models.CATEGORY.create({ name: 'Thông Báo' });
    await models.CATEGORY.create({ name: 'Khác' });
